...
export function insertAtCursor(nodes: Node[], rangeIdx = 0, triggerChange = false) {
  ...
  if (triggerChange) {
    emitter.emit("TRIGGER_EDITABLEDIV_CHANGE", "editable-div");
  }
  return range;
}
...
export default class EditableDiv extends React.Component<Props> {
  ...
  componentDidMount(): void {
    emitter.on("TRIGGER_EDITABLEDIV_CHANGE", this.triggerChange);
  }
  componentWillUnmount(): void {
    emitter.off("TRIGGER_EDITABLEDIV_CHANGE", this.triggerChange);
  }

  triggerChange = (id: string) => {
    const el = this.getEl();
    if (!el) return;
    if (id === el.id) {
      this.emitChange({} as EditableDivEvent);
    }
  };
  ...
  pasteLint = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    //   console.log("xxx text/html", text);
    ...
    if (clp?.items[0].type.includes("text/plain")) {
      ...
      const parser = new DOMParser();
      const formatText = formatEmoji(text);
      if (formatText.includes("emoji-inline")) {
        const doc = parser.parseFromString(formatText, "text/html");
        const nodes = Array.from(doc.body.childNodes);
        insertAtCursor(nodes);
      } else {
    ...
    imageItems.length > 0 && e.preventDefault();
    imageItems.map((item: DataTransferItem) => {
      ...
      reader.onload = () => {
        ...
        image.className = "image-el";
        images.push(image);
        if (images.length === imageItems.length) {
          insertAtCursor([...images], undefined, true);
        }
      };
  ...
  emitChange = (originalEvt: EditableDivEvent) => {
    const el = this.getEl();
    if (!el) return;
